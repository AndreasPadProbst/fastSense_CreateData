            ...
            if isinstance(node, mwp.nodes.external_link.ExternalLink):
                ...
                if node.tag == "br":
                    ...
                    if node.contents is not None:
                        tag_text, tag_links = self.get_text_and_links(node.contents.nodes,
                                                                      link_offset=link_offset + len(text))
                        text += tag_text
                        links += tag_links
        ...
        for i in range(len(split_text)):
            if i % 2 == 0:  # Every second item is whitespace
                ...
                if len(clean_paragraph_text) > 0:
                    if re.search(
                            r"({{)|(}})|({\|)|(\|})|(\[\[)|(\]\])|(\|-)|(^\|)|(align=)|(class=)|(style=)|(cellpadding=)|(cellspacing=)|(border=)|(<div)|(<span)|(<table)|(</)",
                            clean_paragraph_text) is not None:
                        skipped_paragraphs_count += 1
                    elif estimated_token_count < 5 or len(clean_paragraph_text) < 15 or len(
                            re.findall(r"[A-Za-z]", clean_paragraph_text)) / len(clean_paragraph_text) < 0.5:
                        skipped_paragraphs_count += 1
                    else:
                        paragraph = Paragraph()
